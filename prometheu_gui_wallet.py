...
def criar_carteira():
    ...
    with open(f"{os.getenv('PROMETHEU_DIR')}/mnemonics", "w") as f:
        f.write(" ".join(mnemonics))
    ...
    # Envia 1 BTC para carteira lightning
...
def handle_register():
    ...
    with open(f"{os.getenv('PROMETHEU_DIR')}/pin", "w") as f:
...
def attempt_login_callback(sender, app_data, user_data):
    ...
    try:
        with open(f"{os.getenv('PROMETHEU_DIR')}/pin", "r") as f:
    ...
    if hash_sha256(entered) == stored_hash:
        ...
        if state["wrong_attempts"] >= state["max_attempts"]:
            prometheu_dir = os.getenv("PROMETHEU_DIR")
            for arquivo in [f"{prometheu_dir}/pin", f"{prometheu_dir}/mnemonics"]:
